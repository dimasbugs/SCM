			...
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				...
				if($name != null && $phone != null && $resultValidate_email ==1) {
					...
					if(mysqli_query($con,$query_UpdateDist)) {
						echo "<script> alert(\"Detail Distributor telah di update\"); </script>";
						header('Refresh:0;url=view_distributor.php');
					}
					else {
						$requireErr = "Update Distributor gagal";
					}
				}
				else {
...
<body>
	...
	<section>
		...
		<li>
			...
			<div class="input-box"> <input type="text" id="distributor:name" name="txtDistributorName" placeholder="Nama" value="<?php echo $row_selectDistDetails['dist_name']; ?>" required /> </div> <span class="error_message"><?php echo $nameErr; ?></span>
		</li>
		<li>
		...
		<li>
			...
			<div class="input-box"> <input type="text" id="distributor:phone" name="txtDistributorPhone" placeholder="No. Telp" value="<?php echo $row_selectDistDetails['dist_phone']; ?>" /> </div> <span class="error_message"><?php echo $phoneErr; ?></span>
		</li>
		<li>
			<div class="label-block"> <label for="distributor:address">Alamat</label> </div>
			<div class="input-box"> <textarea type="text" id="distributor:address" name="txtDistributorAddress" placeholder="Alamat"><?php echo $row_selectDistDetails['dist_address']; ?></textarea> </div>
		</li>
		<li>
