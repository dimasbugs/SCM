			...
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				...
				if($areaName != null) {
					...
					if(mysqli_query($con,$query_UpdateArea)) {
						...
						$requireErr = "Update Area Gagal";
					}
				}
				else {
					$requireErr = "* Butuh nama area yang Valid";
				}
			}
		}
		else {
...
<body>
	...
	<section>
		...
		<li>
			...
			<div class="input-box"> <input type="text" id="areaName" name="txtAreaName" placeholder="Nama Area" value="<?php echo $row_selectAreaDetails['area_name']; ?>" required /> </div> <span class="error_message"><?php echo $areaNameErr; ?></span>
		</li>
		<li>
			<div class="label-block"> <label for="areaCode">Kode Area</label> </div>
			<div class="input-box"> <input type="text" id="areaCode" name="txtAreaCode" placeholder="Kode Area" value="<?php echo $row_selectAreaDetails['area_code']; ?>" required /> </div>
		</li>
		<li>
