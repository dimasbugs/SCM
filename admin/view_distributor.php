		...
		if($_SESSION['admin_login'] == true) {
			...
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				if(isset($_POST['chkId'])) {
					...
					if(!$result) {
						echo "<script> alert(\"Tidak dapat menghapus distributor yang terpakai\"); </script>";
						header('Refresh:0');
					}
					else {
						echo "<script> alert(\"Distributor berhasil dihapus\"); </script>";
						header('Refresh:0');
					}
				}
			}
