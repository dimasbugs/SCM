			...
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				...
				if($name != null && $email != null && $username != null) {
					...
					if(mysqli_query($con,$query_UpdateMan)) {
						echo "<script> alert(\"Update Store berhasil\"); </script>";
						header('Refresh:0;url=view_manufacturer.php');
					}
					else {
						$requireErr = "Update Store gagal";
					}
				}
				else {
					$requireErr = "* Nama, Email, Username atau Password tidak valid";
				}
			}
		}
		else {
...
<body>
	...
	<section>
		...
		<li>
			...
			<div class="input-box"> <input type="text" id="manufacturer:phone" name="txtManufacturerPhone" placeholder="No. Telp" value="<?php echo $row_selectManDetails['man_phone']; ?>" /> </div> <span class="error_message"><?php echo $phoneErr; ?></span>
		</li>
		<li>
		...
		<li>
			<input type="submit" value="Update Store" class="submit_button" /> <span class="error_message"> <?php echo $requireErr; ?> </span><span class="confirm_message"> <?php echo $confirmMessage; ?> </span>
		</li>
		</ul>
		</form>
	</section>
