		...
		<table class="table_invoiceFormat">
			...
			<?php
				if($row_selectManOrder['approved'] == 0) {
					?>
				<a href="confirm_order.php?id=<?php echo $order_id; ?>"><input type="button" value="Konfirmasi" class="submit_button"/></a>
				<?php
				}
				else {
					if($row_selectManOrder['status'] == 0) {
				?>
					<a href="generate_invoice.php?id=<?php echo $order_id; ?>"><input type="button" value="Buat Invoice" class="submit_button"/></a>
				<?php
					}
					else {
					}
